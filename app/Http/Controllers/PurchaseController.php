    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            foreach ($product_id as $i => $id) {
                ...
                $total_qty = $lims_product_data->qty + $quantity;
                $prev_product_qty = $lims_product_data->qty;
                $prev_product_cost = $lims_product_data->cost;
                $new_product_qty = $quantity;
                $new_product_cost =
                    $net_unit_cost[$i];
                $new_final_cost = (($prev_product_qty * $prev_product_cost) + ($new_product_qty * $new_product_cost)) / $total_qty;
                $lims_product_data->qty = $lims_product_data->qty + $quantity;

                // update product price
                $lims_product_data->cost = round($new_final_cost, 2);
