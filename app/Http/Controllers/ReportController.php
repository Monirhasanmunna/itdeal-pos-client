...
class ReportController extends Controller
{
    public function profit_report()
    {
        ...
        $total_sale = Product_Sale::whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)->sum('total');
        $total_cost = Product_Sale::whereDate('product_sales.created_at', '>=', $start_date)
            ->whereDate('product_sales.created_at', '<=', $end_date)
            ->selectRaw('SUM(product_sales.unit_cost * product_sales.qty) as total_cost')
            ->value('total_cost');
        $total_expense = Expense::whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)->sum('amount');

        return view('backend.report.profit_report', compact('start_date', 'end_date', 'total_sale', 'total_cost', 'total_expense'));
    }
    ...
    public function product_ledger()
    {
        ...
        foreach ($products as $product) {
            // set opening stock
            $prev_purchase_qty = ProductPurchase::where('product_id', $product->id)
            ...
            $prev_sale_qty = Product_Sale::where('product_id', $product->id)
                ->join('sales', 'sales.id', '=', 'product_sales.sale_id')
                ->whereDate('sales.created_at', '<', $start_date)
                ->sum('product_sales.qty');
            $opening_stock = round($prev_purchase_qty - $prev_sale_qty, 2);
            $product->opening_stock = $opening_stock;

            $purchase_qty = ProductPurchase::where('product_id', $product->id)
                ->join('purchases', 'purchases.id', '=', 'product_purchases.purchase_id')
                ->whereDate('purchases.created_at', '>=', $start_date)
                ->whereDate('purchases.created_at', '<=', $end_date)
                ->sum('product_purchases.qty');
            $product->purchase_qty = $purchase_qty;

            $sale_qty = Product_Sale::where('product_id', $product->id)
                ->join('sales', 'sales.id', '=', 'product_sales.sale_id')
                ->whereDate('sales.created_at', '>=', $start_date)
            ...
            $adjustment_plus = ProductAdjustment::whereDate('created_at', '>=', $start_date)
                ->whereDate('created_at', '<=', $end_date)
                ->where('product_id', $product->id)
                ->where('action', '+')
                ->sum('qty');
            $product->adjustment_plus = $adjustment_plus;

            $adjustment_minus = ProductAdjustment::whereDate('created_at', '>=', $start_date)
                ->whereDate('created_at', '<=', $end_date)
                ->where('product_id', $product->id)
                ->where('action', '-')
                ->sum('qty');
            $product->adjustment_minus = $adjustment_minus;

            $return_qty = ProductReturn::whereDate('created_at', '>=', $start_date)
                ->whereDate('created_at', '<=', $end_date)
                ->where('product_id', $product->id)
                ->sum('qty');
            $product->return_qty = $return_qty;

            $closing_stock = $opening_stock + $purchase_qty - $sale_qty + $return_qty + $adjustment_plus - $adjustment_minus;

            $product->closing_stock = $closing_stock;
            $product->stock_price = round($closing_stock * $product->price, 2);
        }

        return view('backend.report.product_ledger', compact('products', 'start_date', 'end_date'));
    }
    ...
    public function customer_ledger()
    {
        ...
        if (!$customer_id) {
            return view('backend.report.customer_ledger', compact('start_date', 'end_date', 'customers', 'items'));
        }
        ...
        $sales = Sale::where('customer_id', $customer_id)
            ...
            ->get()
            ->each(function ($sale) {
                $sale->particular = 'Sale';
                $sale->type = 'debit';
                $sale->amount = $sale->grand_total;
            });

        $payments = Payment::join('sales', 'sales.id', 'payments.sale_id')
            ->where('sales.customer_id', $customer_id)
            ->when($start_date, function ($query) use ($start_date) {
                $query->whereDate('payments.created_at', '>=', $start_date);
            })
            ->when($end_date, function ($query) use ($end_date) {
                $query->whereDate('payments.created_at', '<=', $end_date);
            })
            ->get()
        ...
        $items = collect($sales)->merge($payments);
        $items = $items->sortBy(function ($item) {
            // First, sort by created_at in ascending order
            $result = $item->created_at;

            // If created_at is the same, put debit (payments) before credit (sales)
            if ($item->type === 'debit') {
                $result .= '0'; // Add a suffix to make debit come first
            } else {
                $result .= '1'; // Add a suffix to make credit come after debit
            }

            return $result;
        });

        return view('backend.report.customer_ledger', compact('start_date', 'end_date', 'items', 'customers'));
    }
    public function customer_product()
    {
        ...
        foreach ($products as $product) {
            $sale_qty = Product_Sale::where('product_sales.product_id', $product->id)
                ->join('sales', 'sales.id', '=', 'product_sales.sale_id')
                ->whereDate('sales.created_at', '>=', $start_date)
                ->whereDate('sales.created_at', '<=', $end_date)
                ->where('sales.customer_id', $customer_id)
            ...
            $price = Product_Sale::where('product_sales.product_id', $product->id)
                ->join('sales', 'sales.id', '=', 'product_sales.sale_id')
                ->whereDate('sales.created_at', '>=', $start_date)
                ->whereDate('sales.created_at', '<=', $end_date)
                ->where('sales.customer_id', $customer_id)
            ...
            $unit_name = Product::join('units', 'units.id', 'products.sale_unit_id')->where("products.id", $product->id)->value('units.unit_code');
            $product->sale_qty = round($sale_qty, 2);
            $product->sale_price = $price;
            $product->unit_name = $unit_name;
        }

        $products = $products->filter(function ($product) {
            return $product->sale_qty > 0 || $product->sale_price > 0;
        });

        if (!$customer_id) {
            $products = [];
            return view('backend.report.customer_product_report', compact('start_date', 'end_date', 'customers', 'products'));
        }
    ...
    public function product_customer()
    {
        ...
        $customers = Customer::latest()->get();
        $products = Product::where('is_active', 1)->get();
        $unit_name = Product::join('units', 'units.id', 'products.sale_unit_id')->where("products.id", $product_id)->value('units.unit_code');

        foreach ($customers as $customer) {
            $sale_qty = Product_Sale::where('sales.customer_id', $customer->id)
                ->join('sales', 'sales.id', '=', 'product_sales.sale_id')
                ->whereDate('sales.created_at', '>=', $start_date)
                ->whereDate('sales.created_at', '<=', $end_date)
            ...
            $price = Product_Sale::where('sales.customer_id', $customer->id)
                ->join('sales', 'sales.id', '=', 'product_sales.sale_id')
                ->whereDate('sales.created_at', '>=', $start_date)
                ->whereDate('sales.created_at', '<=', $end_date)
                ->where('product_sales.product_id', $product_id)
            ...
            $customer->sale_qty = round($sale_qty, 2);
            $customer->sale_price = $price;
        }

        $customers = $customers->filter(function ($customer) {
            return $customer->sale_qty > 0 || $customer->sale_price > 0;
        });
        ...
        if (!$product_id) {
            $customers = [];
            $unit_name = '';
            return view('backend.report.product_customer_report', compact('start_date', 'end_date', 'customers', 'products', 'unit_name'));
        }
    ...
    public function cashbook()
    {
        $start_date = request()->start_date ?? date('Y-m-d');
        $end_date = request()->end_date ?? date('Y-m-d');
        $next_date = Carbon::parse($end_date)->addDays(1);

        $previous_sale_paid_amount = Sale::whereDate('created_at', '<', $start_date)->sum('paid_amount');
        $previous_expense_amount = Expense::whereDate('created_at', '<', $start_date)->sum('amount');
        $previous_purchase_amount = Purchase::whereDate('created_at', '<', $start_date)->sum('paid_amount');
        $previous_sale_return_amount = ProductReturn::whereDate('created_at', '<', $start_date)->sum('total');
        $opening_balance = $previous_sale_paid_amount - $previous_expense_amount - $previous_purchase_amount - $previous_sale_return_amount;

        $total_sale_amount = Sale::whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)->sum('grand_total');
        $total_sale_paid_amount = Sale::whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)->sum('paid_amount');
        $total_purchase_amount = Purchase::whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)->sum('grand_total');
        $total_purchase_paid_amount = Purchase::whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)->sum('paid_amount');
        $previous_due_collection = Payment::whereDate('payments.created_at', '>=', $start_date)
        ...
        $sale_return_amount = ProductReturn::whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)->sum('total');

        $total_debit = $total_sale_paid_amount + $previous_due_collection + $opening_balance;
        $total_credit = $total_purchase_paid_amount + $expense_amount;

        $balance_cd = $total_debit - $total_credit;

        $data = compact(
            'start_date',
            'end_date',
            'total_sale_amount',
            'total_sale_paid_amount',
            'total_purchase_amount',
            'total_purchase_paid_amount',
            'previous_due_collection',
            'opening_balance',
            'total_debit',
            'expense_amount',
            'sale_return_amount',
        ...
        return view('backend.report.cashbook_bad', $data);


        /**
         * The following code were standard
         */
        //     $balance_bd = 0;
        //     $balance_cd = 0;
        //     $items = [];

        //     $sale_items = Sale::whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)->get();
        //     $purchase_items = Purchase::whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)->get();
        //     $expenses = Expense::whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)->get();
        //     $salePayments = Payment::join('sales', 'sales.id', 'payments.sale_id')
        //         ->whereNotNull('payments.sale_id')
        //         ->where('sales.created_at', '<', 'payments.created_at')
        ...
        //         $items[] = (object) [
        //             'date' => $salePayment->created_at,
        //             'amount' => $salePayment->amount,
        //             'type' => 'Debit',
        //             'particular' => 'Due Collection'
        //         ];
        //     }
        ...
        //     $debitItems = $items['Debit'] ?? collect();
        //     $creditItems = $items['Credit'] ?? collect();

        //     // Add 'Balance B/D' and 'Balance C/D' items to the respective collections
        //     $debitItems = $debitItems->prepend((object) ['date' => $start_date, 'particular' => "Balance B/D", 'amount' => $balance_bd]);
        //     $creditItems = $creditItems->push((object) ['date' => $end_date, 'particular' => "Balance C/D", 'amount' => $balance_cd]);

        //     // Update the $items array with the modified 'Debit' and 'Credit' collections

        //     return view('backend.report.cashbook_bad', compact('start_date', 'end_date', 'items'));
    }
